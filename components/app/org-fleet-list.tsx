...
/** Full aircraft list for the fleet page (links to existing aircraft detail). */
export default function OrgFleetList({
  organisationId,
  aircraft,
  canManage,
}: {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-border/60 bg-white/80 shadow-[0_1px_0_color-mix(in_oklch,white_40%,transparent)_inset,0_8px_30px_-24px_color-mix(in_oklch,var(--primary)_30%,transparent)]">
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-border/50 px-6 py-5">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {aircraft.length} registered · Select a row for details
          </p>
        </div>
      ...
      <div className="p-2">
        {aircraft.length === 0 ? (
          <div className="m-4 flex min-h-40 items-center justify-center rounded-xl border border-dashed border-border/60 bg-white/40 px-6 py-8 text-center text-sm text-muted-foreground">
            ...
            {canManage ? " Use “Add aircraft” above to register one." : null}
          </div>
        ) : (
          <ul className="divide-y divide-border/50">
            {aircraft.map((a, i) => (
              <li
                key={a.id}
                className={cn("rise-in", i < 6 && `rise-in-${i + 1}`)}
              >
                <Link

...
The parse flight plan flow ingests a **single flight-plan PDF** (typically ForeFlight-style exports), uses an **LLM-based PDF splitter** to classify every page, **physically splits** the PDF into a flight-details document and one-or-more NOTAM-section documents, then runs **two largely independent tracks**:

| Track | Input | Model(s) | Persists when | Written to |
|--------|--------|----------|----------------|------------|
| **Flight data** | Flight-details PDF (Anthropic file) | Claude **Sonnet 4.6** | Before the HTTP response returns | `flights` |
...
flowchart TB
  ...
  subgraph flight["Flight track — awaited"]
    E[Upload flight-details PDF]
    F[Flight data extraction agent]
    G[buildPersistedFields + applyParsedFlightPlanToFlight]
  end

  subgraph notamBg["NOTAM track — background"]
    H[pdf-parse per NOTAM PDF]
    I[Join text]
    J[splitNotamText + parallel runNotamExtractionAgent]
    K[mergeNotamExtractionPartials — no LLM]
    L[extractedNotamsToRawPayload + upsertPendingNotamAnalysis]
  end

  subgraph response["Response"]
    M[markPendingNotamExtraction then JSON 200]
    N[finally: delete uploaded file ids]
  end
...
- **Output:** Structured JSON validated against **`splitterResultSchema`** (`lib/flight-plan/schemas.ts`): **`SplitterResult`** with:
  ...
  - **`flightDetailPages`**: all other non-NOTAM, non–wind-map pages.
- **Post-processing:** **`clampSplitterResult`** deduplicates, sorts, and drops out-of-range page numbers.
...
- **Wind maps** are excluded by construction — they appear neither in NOTAM groups nor in **`flightDetailPages`**.
...
3. **`buildFullExtractionFromFlightData`** — fills **`extracted_notams`** with empty arrays for typing, then **`buildPersistedFields`** applies ICAO normalization, **`status: "draft"`**, masks unidentified top-level fields, forces **`flight_plan_json: null`**, and sets **`pdf_file_id`** to the **original** upload id.
4. **`applyParsedFlightPlanToFlight`** writes to **`flights`**.

If there is **no** flight-details PDF, the route uses **`FLIGHT_DATA_FALLBACK_PARTIAL`** (all null flight fields + every flight key listed in **`unidentified_fields`**).
...
| **Structured output** | **`zodOutputFormat`** on a schema derived from **`flightDataExtractionPartialSchema`** with **`flight_plan_json` omitted** — the model does not return waypoint tables. |
| **Input modality** | **`document`** + **`file_id`** for the uploaded **flight-details** PDF only. |

The agent returns **`FlightDataExtractionPartial`**. The route then **injects** empty **`flight_plan_json`** at the type level via **`buildFullExtractionFromFlightData`** before **`buildPersistedFields`**, which **always** sets **`flight_plan_json: null`** on the persisted **`FlightPlanParsedFields`**.
...
4. **`Promise.all`** in parallel:
   ...
   - **`runFlightDataExtractionAgent`** on uploaded flight-details PDF (or fallback partial).
5. **`runSupervisorAgent`** — **Claude Haiku 4.5**, streaming **`finalMessage()`**, **`zodOutputFormat(flightPlanExtractionSchema)`** — merges **`flightDataPartial`** + **`notamPartial`** into a **full** **`FlightPlanExtraction`** (this path **does** use an LLM merge).
